...
/**
 * ✅ Version "clean" :
 * - Page Ajouter : recherche + catégorie + select repas en haut
 * - Favoris inchangé
 * - Clic sur ➕ : ouvre un modal quantité/unité (g/ml/portion)
 * - Ajout : reste sur la page Ajouter (pas de retour dashboard)
 * 
 * ⚠️ Prérequis HTML :
 * - Sur la page ADD : il ne doit plus y avoir #qtyInput / #qtyUnit (puisqu'on passe par le modal)
 * - Le modal doit exister dans index.html (ids: addModal, modalQty, modalUnit, modalConfirm, modalCancel, addModalClose, addModalBackdrop, addModalTitle, addModalSub, modalPreview)
 */
...
const state = {
  ...
  dayKey: dateKey(),
  activeMeal: "breakfast",     // meal affiché sur le dashboard
  addMeal: "breakfast",        // meal cible quand on ajoute depuis "Ajouter"
...
function renderRing(sumKcal, goalKcal) {
  ...
  const deg = Math.round((pct / 100) * 360);
  // utilise une couleur neutre (border-ish) pour le reste
  ring.style.background = `conic-gradient(var(--accent) ${deg}deg, rgba(18,50,60,.12) 0deg)`;
}
...
function openAddModal(food) {
  ...
  $("#addModalTitle").textContent = `Ajouter : ${food.name}`;
  $("#addModalSub").textContent = `${food.kcal} kcal / ${food.unit || "100g"} • ${food.categoryLabel}`;
...
function updateModalPreview() {
  ...
  const macros = computePortionMacros(food, qty, unit);
  $("#modalPreview").textContent =
    `${qty}${unit} → ${macros.kcal} kcal | P ${macros.protein}g • G ${macros.carbs}g • L ${macros.fat}g`;
}
...
function confirmModalAdd() {
  ...
  // ✅ Ajout dans le repas sélectionné en haut
  addJournalEntry(state.dayKey, state.addMeal, entry);

  // ✅ On reste sur Ajouter (pas de setView("dashboard"))
  closeAddModal();

  // refresh global (dashboard totals, favoris, etc.)
  renderAll();
}
...
function renderFavoritesBlock(targetEl, { allowAdd = true } = {}) {
  ...
  for (const f of favFoods) {
    const el = createItem({
      ...
      onAdd: allowAdd ? () => openAddModal(f) : null
    });
...
function bindEvents() {
  ...
  // dashboard meal select
  $("#mealSelect").addEventListener("change", (e) => {
    setMeal(e.target.value);
  });

  // dashboard meal chips
  $("#mealTabs").addEventListener("click", (e) => {
    const btn = e.target.closest(".chip");
    if (!btn) return;
...
async function init() {
  ...
  if (!state.foods.length) {
    console.warn("Aucun aliment chargé. Vérifie /data/aliments/*.json");
  }
